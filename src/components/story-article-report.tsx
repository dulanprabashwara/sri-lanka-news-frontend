...
export function StoryArticleReport({ article }: { article: Article }) {
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2 text-xs font-semibold uppercase tracking-wide text-slate-500">
        <Link href={`/source/${encodeURIComponent(article.source.slug)}`} className="text-teal-700 hover:underline">
          {article.source.name}
        </Link>
      ...
      <h2 className="mt-3 text-xl font-bold tracking-tight text-slate-950">
        <Link href={`/article/${encodeURIComponent(article.id)}`} className="hover:text-teal-800 hover:underline">
          {article.title}
        </Link>
      </h2>
      {article.summary ? (
