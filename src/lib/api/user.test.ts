...
test("parses defaults and maps a saved display preference", () => {
  const preferences = parsePreferences({
    preferredDisplayLanguage: "ORIGINAL",
    preferredCategories: [],
    createdAt: null,
    updatedAt: null,
  });
  assert.equal(
    preferredDisplayLanguage(preferences.preferredDisplayLanguage),
    undefined,
  );
...
test("rejects malformed preference and bookmark status payloads", () => {
  assert.throws(() =>
    parsePreferences({
      preferredDisplayLanguage: "INVALID",
      preferredCategories: [],
    }),
  );
...
test("uses protected preference and bookmark requests without owner data", async () => {
  ...
  globalThis.fetch = (async (
    ...
    if (new URL(String(input)).pathname.endsWith("preferences"))
      return Response.json({
        preferredDisplayLanguage: "EN",
        preferredCategories: ["LOCAL"],
        createdAt: "2026-09-02T00:00:00Z",
        updatedAt: "2026-09-02T00:00:00Z",
      });
    if (new URL(String(input)).pathname.endsWith("bookmarks"))
      return Response.json({
        content: [],
        page: 0,
        size: 20,
  ...
  assert.deepEqual(
    ...
    [
      ...
      [
        "/api/v1/me/bookmarks?page=0&size=20&type=ARTICLE&displayLanguage=si",
        "GET",
      ],
    ],
  );
  assert.ok(
    requests.every((request) => request.authorization === "Bearer jwt"),
  );
...
test("parses source/topic follows and batched status without private keys", () => {
  ...
  assert.equal(page.content[0]?.newArticleCount, 5);
  assert.equal(page.content[1]?.topic?.label, "Cricket");
  assert.equal(page.content[1]?.newArticleCount, null);
  ...
  assert.deepEqual(
    parseFollowBatchStatus({
      ...
      topics: [{ topic: "Cricket", followed: false, followedAt: null }],
    }).topics[0],
    { topic: "Cricket", followed: false, followedAt: null },
  );
});
...
test("uses one protected batch request and owner-free follow mutations", async () => {
  ...
  globalThis.fetch = (async (
    ...
    if (
      init?.method === "DELETE" ||
      (init?.method === "POST" && url.pathname.endsWith("/seen"))
    )
    ...
    if (url.pathname === "/api/v1/me/follows")
      return Response.json({
        content: [],
        page: 0,
        size: 20,
  ...
  await markSourceSeen("jwt", "daily-mirror");
  ...
  assert.deepEqual(
    ...
    [
      ...
      ["/api/v1/me/follows/sources/daily-mirror/seen", "POST"],
  ...
  assert.deepEqual(requests[3]?.body, { topic: "Drug Trafficking" });
  assert.deepEqual(requests[5]?.body, {
    sourceSlugs: [],
    topics: ["Cricket", "Elections"],
  });
  assert.ok(
    requests.every(
      (request) =>
        request.body == null ||
        !("userId" in (request.body as Record<string, unknown>)),
    ),
  );
});
    ...
    content: [
      {
        article: {
          ...
          discoveredAt: "2026-09-02T10:01:00Z",
          category: "LOCAL",
          summary: null,
          topics: [],
          source: {
      ...
      },
      {
        article: {
          ...
          discoveredAt: "2026-09-02T09:01:00Z",
          category: null,
          summary: null,
          topics: [],
          source: {
...
test("uses the protected For You endpoint with bounded pagination and language", async () => {
  ...
  globalThis.fetch = (async (
    ...
    return Response.json({
      ...
      first: false,
      last: true,
      personalization: { personalized: false, signalCount: 0 },
    });
  }) as typeof fetch;
  const feed = await getForYouFeed("jwt", { page: 2, displayLanguage: "ta" });
  assert.deepEqual(request, {
    path: "/api/v1/me/for-you?page=2&size=20&displayLanguage=ta",
